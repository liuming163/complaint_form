...
def xiaohongshu_page():
    return render_template('xiaohongshu.html')
...
def run_xiaohongshu_complaint_script(task_id, cookie, principal_name, works_config, total_batches):
    import sys, tempfile
    script_path = os.path.join(os.path.dirname(__file__), 'xiaohongshu_complaint_backend.py')
    submission_id = task_id[len('xiaohongshu_'):] if task_id.startswith('xiaohongshu_') else task_id
...
from xiaohongshu_routes import xiaohongshu_bp
